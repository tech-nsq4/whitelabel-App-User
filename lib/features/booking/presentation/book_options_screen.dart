...
/// The legacy "how do you want to book" chooser — by specialty / by doctor /
/// by branch, plus the symptom checker. Kept intact but off the main flow:
/// `HomeServicesGrid` → [BookScreen] (the specialty grid) is the entry point
/// now. Reachable via [Routes.bookOptions] if a caller wants the old chooser.
class BookOptionsScreen extends StatefulWidget {
  const BookOptionsScreen({super.key});

  @override
  State<BookOptionsScreen> createState() => _BookOptionsScreenState();
}

class _BookOptionsScreenState extends State<BookOptionsScreen> {
  int _tab = 0;
