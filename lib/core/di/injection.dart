...
import '../../features/account/logic/contact_cubit.dart';
import '../../features/auth/data/auth_repo.dart';
import '../../features/auth/logic/auth_cubit.dart';
import '../../features/banners/data/banners_repo.dart';
import '../../features/banners/logic/banners_cubit.dart';
...
import '../../features/booking/logic/doctor_reviews_cubit.dart';
...
import '../../features/booking/logic/quote_cubit.dart';
...
import '../../features/pages/data/pages_repo.dart';
import '../../features/pages/logic/page_cubit.dart';
...
Future<void> setupDi() async {
  ...
  getIt.registerLazySingleton(() => AccountRepo(dio: getIt()));
  getIt.registerLazySingleton(() => BannersRepo(dio: getIt()));
  ...
  getIt.registerLazySingleton(() => PagesRepo(dio: getIt()));
  getIt.registerLazySingleton(() => OnboardingRepo(dio: getIt()));

  // ─── Cubits ───────────────────────────────────────────────────────────────
  getIt.registerFactory(() => AuthCubit(getIt()));
  getIt.registerFactory(() => ContactCubit(getIt()));
  getIt.registerFactory(() => BannersCubit(getIt()));
  ...
  getIt.registerFactory(() => DoctorReviewsCubit(getIt()));
  ...
  getIt.registerFactory(() => PageCubit(getIt()));
  ...
  // Singleton for the same reason: the heart toggle on every `BranchCard`
  // (across `BranchesScreen`, `FavoritesScreen`, ...) reads and mutates this
  // one instance, so a branch favorited in one place shows favorited
  // everywhere. Provided app-wide from `MyApp`; loaded by `LayoutScreen`.
